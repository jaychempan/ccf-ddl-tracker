...
同时也提供了适用于分支部署模式的 [`docs/`](docs/) 镜像目录。

1. 将仓库推送到 GitHub 的 `main` 分支。
2. 打开 `Settings -> Pages`。
3. 二选一即可：
4. 选择 `GitHub Actions`，直接从 `website/` 发布。
5. 或选择 `Deploy from a branch`，然后使用 `main` 分支下的 `/docs`。

如果你使用 `/docs` 分支部署模式，并且更新了 [`website/`](website/) 中的网页内容，请运行：
