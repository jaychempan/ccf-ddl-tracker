...
当前源码还包含尚未发布的夜间模式设置，可通过“加载已解压的扩展程序”体验；推送源码到 GitHub 不会更新 Chrome 应用商店安装包。
...
如果 macOS 上点击工具栏图标迟迟不弹出，或“审查弹出内容”也打不开，请查看[弹窗卡顿排查指南](chrome/POPUP-TROUBLESHOOTING.md#中文)。其中说明了 Chrome 重启、临时启动参数 `PMLoadingPageVoter` 和本次验证的局限。更新扩展不会自动启用这个浏览器参数。

## 更新日志

<details open>
  <summary><strong>尚未发布</strong> - 外观设置与弹窗卡顿排查</summary>
