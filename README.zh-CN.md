...
仓库现在已经包含基于 GitHub Actions 的 Pages 部署流程，发布内容来自 [`website/`](website/)。
[`docs/`](docs/) 目录只保留为旧的分支部署模式兼容备用。

1. 将仓库推送到 GitHub 的 `main` 分支。
2. 打开 `Settings -> Pages`。
3. 将 `Source` 设置为 `GitHub Actions`。
4. 保持 [`.github/workflows/deploy-pages.yml`](.github/workflows/deploy-pages.yml) 处于启用状态。
5. 向 `main` 推送变更，或者在 `Actions` 页面手动运行 `Deploy GitHub Pages` 工作流。

如果你暂时仍使用 `/docs` 分支部署模式，并且更新了 [`website/`](website/) 中的网页内容，请运行：
